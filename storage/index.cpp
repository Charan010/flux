...
constexpr char     kMagic[8]         = {'R','L','C','K','P','T','0','1'};
constexpr uint32_t kVersion          = 1;
constexpr size_t   kHeaderSize       = 8 + 4 + 8 + 8 + 32;  /* magic, version, count, lsn, blake3 */
constexpr size_t   kRecordSize       = 32 + 4 + 8;
...
void Index::recover() {
	...
	/* A crash between opening the tmp checkpoint and renaming it leaves an
	   orphan behind. Harmless, but it would accumulate. */
	std::error_code ec;
	fs::remove(checkpoint_path_.string() + ".tmp", ec);
...
uint64_t Index::load_checkpoint() {
	...
	if (!in)
		throw std::runtime_error("index: cannot open checkpoint");
...
void Index::checkpoint() {
	...
	/* Checkpoint is written first to the disk and then log is truncated or resetted. */

...
std::string ObjectStore::store(const Chunk &chunk){
	...
	rotate_pack_if_needed(chunk.bytes.size());
	uint64_t offset = current_pack_offset_;
 
	current_pack_.write(reinterpret_cast<const char*>(chunk.bytes.data()), chunk.bytes.size());
 
	if (!current_pack_)
		throw std::runtime_error("Failed to write chunk to pack file.");
 
	current_pack_.flush();
	current_pack_offset_ += chunk.bytes.size();
 
	ObjectLocation location{};
	location.pack_id = current_pack_id_;
	location.offset = offset;
	location.size = static_cast<uint32_t>(chunk.bytes.size());
...
Chunk ObjectStore::load(const std::string &digest) const{
	...
	const ObjectLocation *location = index_.find(digest);
	...
	Chunk chunk;
	chunk.bytes.resize(location->size);
 
	in.read(reinterpret_cast<char*>(chunk.bytes.data()), location->size);

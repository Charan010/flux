...
ChunkBuffer::ChunkBuffer(BitWriter& bw, uint32_t total_chunks, ProgressBar* progress,
                         bool write_headers)
    : bw(bw), total_chunks(total_chunks), progress(progress), write_headers(write_headers),
      slots(new Slot[total_chunks]) {

    writer_thread = std::thread(&ChunkBuffer::writer_loop, this);
}

ChunkBuffer::~ChunkBuffer() { finish(); }

/*
    writer_loop()

    @brief It is a single thread loop which gets woken up everytime threadpool submits a task.
    and checks if it is the expected_chunk or not. If yes, it gets written to the file using
   BitWriter class which abstracts std::ofstream.
...
void ChunkBuffer::writer_loop() {
    ...
    while (next < total_chunks) {
        ...
        if (slot.state.load(std::memory_order_acquire) != SlotState::Filled) {
            ...
            sleep_cv.wait(lock, [&] {
                return slot.state.load(std::memory_order_acquire) == SlotState::Filled ||
